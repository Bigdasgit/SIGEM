...
def LINOW_bn_EMB (graph='', dataset_name='', iterations=0, damping_factor=0.8, bch_size_cpu=16, bch_size_gpu=16, prl_num=1, 
    ...
    '''    
    print("Batch similarity computation --> dataset:'{}', iterations:{}, C:{}, batch:{}, {}, {}".format(graph,iterations,damping_factor,(bch_size_gpu if GPU else bch_size_cpu),('#processes:'+str(prl_num) if not GPU else 'single process'),('GPU' if GPU else 'CPU'))+'\n')
    ...
    if (degrees != out_degrees).any(): 
        topK = scaling_factor*avg_degree
    else: 
        topK = scaling_factor*2*avg_degree
    ...
    if GPU: 
        ...
        for idx in tqdm(range(0, len(batches))):
            ...
            with tf.device('/CPU:0'):
                [top_indices.append(result_[j].numpy()) for j in range(result_.nrows())] 
        ...
        for idx in tqdm(range(0, len(batches), prl_num)):                
            with ProcessPoolExecutor(max_workers=prl_num) as executor:
                ...
                for i, future in enumerate(futures):
                    [top_indices.append(future.result()[j]) for j in range(len(future.result()))] 

    ####################################################
        # Save the results if requested
    ####################################################      
    name_ = topK_save_path+dataset_name+'_SIGEM_IT_'+str(iterations)+'_C_'+ str(int(damping_factor*10))+'_'+loss+'_Scl'+str(scaling_factor)
...
if __name__ == "__main__":
    ...
    #### Examples      
    LINOW_bn_EMB(graph="data/Cora_directed_graph.txt",
                      dataset_name = 'Cora',
                      iterations=5,
                      damping_factor = 0.2,
                      bch_size_cpu=2048, 
